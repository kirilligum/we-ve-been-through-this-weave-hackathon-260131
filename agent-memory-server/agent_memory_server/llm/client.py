...
class LLMClient:
    ...
    def create_embeddings(cls) -> Embeddings:
        ...
        from agent_memory_server.llm.embeddings import (
        ...
        # Demo mode: deterministic embeddings without external API calls.
        if os.getenv("WBTT_DEMO_EMBEDDINGS", "").lower() in {"1", "true", "yes"}:
            dims = None
            if embedding_config:
                dims = embedding_config.embedding_dimensions
            elif settings.redisvl_vector_dimensions:
                dims = settings.redisvl_vector_dimensions
            dims = int(dims or 1536)
            logger.warning(
                "WBTT_DEMO_EMBEDDINGS enabled; using deterministic local embeddings "
                "with %d dimensions.",
                dims,
            )
            return DeterministicEmbeddings(dimensions=dims)
