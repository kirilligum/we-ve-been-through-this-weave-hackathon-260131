...
async def search_index(request):
    ...
    # Allow no-Docker runs for WBTT tests
    if os.getenv("WBTT_NO_DOCKER") == "1":
        yield
        return

    async_redis_client = request.getfixturevalue("async_redis_client")
...
@pytest.fixture(autouse=True)
def use_test_redis_connection(request):
